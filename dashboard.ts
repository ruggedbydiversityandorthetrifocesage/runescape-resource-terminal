...
export function initPathfinding(): void {
    ...
    // Pre-allocate underground Mining Guild zones (level 0, z ~9712-9760).
    // Underground coords use z = surface_z + 6400 (e.g. Falador mining guild
    // surface ~3340 → underground ~9740). These zones are outside the mainland
    // range and absent from collision-data.json, so without this the pathfinder
    // returns "Source zone not allocated" for any position inside the guild.
    let undergroundZones = 0;
    for (let x = 3008; x <= 3072; x += 8) {
        for (let z = 9712; z <= 9760; z += 8) {
            if (!rsmod.isZoneAllocated(x, z, 0)) {
                rsmod.allocateIfAbsent(x, z, 0);
                undergroundZones++;
            }
    ...
    initialized = true;
    console.log(`Pathfinding initialized in ${Date.now() - start}ms (${data.zones.length} zones + ${mainlandZones} mainland fill + ${undergroundZones} underground fill, ${data.tiles.length} tiles, ${doorCount} doors masked, ${skippedOneWay} one-way doors blocked)`);
}
